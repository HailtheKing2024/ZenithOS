...
if [ -n "$rootfs" ]; then
    ...
    require_text "$rootfs/etc/gdm3/daemon.conf" "AutomaticLoginEnable=false"
    # Verify login screen configuration (no banner, user list visible)
    require_text "$rootfs/etc/dconf/db/gdm.d/00-zenith-login" "banner-message-enable=false"
